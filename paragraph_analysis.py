...
analysis_print(word_list[:10], analysis_list[:10])
the_dict = count_words(the_data)

print('\nThe highest 10 counts:')
highest_list_10 = sorted(the_dict.items(),key=itemgetter(1),reverse=True)[:10]
tuple_print(highest_list_10)

print('\nThe highest 20 counts:')
highest_list_20 = sorted(the_dict.items(),key=itemgetter(1),reverse=True)[:20]
tuple_print(highest_list_20)